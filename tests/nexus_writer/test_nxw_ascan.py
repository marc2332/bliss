...
def test_nxw_ascan_alt(nexus_writer_config_alt):
    _test_nxw_ascan(**nexus_writer_config_alt, alt=True)


def test_nxw_ascan_nopolicy(nexus_writer_config_nopolicy):
    _test_nxw_ascan(**nexus_writer_config_nopolicy, withpolicy=False)


def test_nxw_ascan_base(nexus_writer_base):
    _test_nxw_ascan(**nexus_writer_base, config=False)


def test_nxw_ascan_base_alt(nexus_writer_base_alt):
    _test_nxw_ascan(**nexus_writer_base_alt, config=False, alt=True)


def test_nxw_ascan_base_nopolicy(nexus_writer_base_nopolicy):
    _test_nxw_ascan(**nexus_writer_base_nopolicy, config=False, withpolicy=False)
