...
class flex(object):
    def __init__(self, name, config):
        ...
        flex_logger.info("")
        flex_logger.info("#" * 50)
        flex_logger.info("pyFlex Initialised")
    ...
    def connect(self):
        flex_logger.info("reading config file")
        ...
        flex_logger.info("connecting to Flex")
        ...
        flex_logger.info("Connection done")
    ...
    def get_detection_param(self, section, name_value):
        ...
        if section == "acq_time":
            flex_logger.info("Acquisition time is set to %s" % (str(val)))
        else:
            flex_logger.info("Roi for %s detection is %s" % (section, str(val)))
        return val
    ...
    def setSpeed(self, speed):
        if 0 <= speed <= 100:
            flex_logger.info("Set speed to %d" % speed)
            self.robot.setSpeed(speed)
            flex_logger.info("Speed is at %s" % str(self.robot.getSpeed()))
    ...
    def enablePower(self, state):
        ...
        if i == 9 and self.robot.getCachedVariable("IsPowered") is not state:
            ...
            flex_logger.error(msg)
            raise RuntimeError(msg)
        flex_logger.info("Power set to %s" % state)
    ...
    def abort(self):
        ...
        flex_logger.info("Robot aborted")

    def set_io(self, dio, boolean):
        flex_logger.info("Set IO %s to %s" % (dio, str(bool(boolean))))
        if bool(boolean):
    ...
    def gripper_port(self, boolean):
        ...
        try:
            ...
            flex_logger.error("Timeout on gripper port")
    ...
    def robot_port(self, boolean):
        ...
        try:
            ...
            flex_logger.error("Timeout on robot port")
    ...
    def moveDewar(self, cell, puck=1, user=False):
        ...
        try:
            ...
            self.stopDewar()
            flex_logger.error("Timeout")
            raise
        flex_logger.info("Dewar moved to %d" % cell)
    ...
    def get_robot_cache_variable(self, varname):
        try:
            # flex_logger.info("cache variable %s is %s", str(varname), str(self.robot.getCachedVariable(varname).getValue()))
    ...
    def dryWithoutPloun(self):
        flex_logger.info("Starting defreeze gripper and stay parked")
        gripper_type = self.get_gripper_type()
        flex_logger.info("gripper type %s" % gripper_type)
        if gripper_type not in [-1, 0, 1, 3, 9]:
            flex_logger.error("No or wrong gripper")
            raise ValueError("No or wrong gripper")
        if gripper_type == -1:
        ...
        if gripper_type in [1, 3, 9]:
            ...
            self.update_transfer_iteration(reset=True)
        flex_logger.info("Defreezing gripper finished")
    ...
    def defreezeGripper(self):
        global DEFREEZING
        DEFREEZING = True
        flex_logger.info("Starting defreeze gripper")
        self.set_io("dioEnablePress", True)
        gripper_type = self.get_gripper_type()
        flex_logger.info("gripper type %s" % gripper_type)
        if gripper_type not in [-1, 0, 1, 3, 9]:
    ...
    def check_coordinates(self, cell, puck, sample):
        if isinstance(cell, int) and isinstance(puck, int) and isinstance(sample, int):
            ...
            sample = sample - 1
            if puck == 23:
                flex_logger.error("cannot load/unload from/to recovery puck")
                raise RuntimeError("cannot load/unload from/to recovery puck")
        else:
            flex_logger.error("cell, puck and sample must be integer")
    ...
    def waiting_for_image(self, acq_time=0.001, timeout=60):
        ...
        flex_logger.info("Image number %d" % prev_imgNb)
        imgNb = prev_imgNb
        try:
            ...
            flex_logger.error("Timeout in waiting for image")
            raise
        flex_logger.info("image taken number %d" % imgNb)
        prev_imgNb = imgNb
        image = self.cam.BW_image(self.cam.take_snapshot())
        try:
            self.cam.image_save(image)
        except Exception as e:
            flex_logger.exception("Could not save image")
        return image

    def save_ref_image(self, image, filename):
        flex_logger.info("Saving the reference file")
        ...
        try:
            ...
            flex_logger.info("error in OS command remove")
        try:
            os.rename(file_path, old_file_path)
        except OSError:
            flex_logger.info("error in OS command rename")
        flex_logger.info("save ref image in %s" % file_path)
        self.cam.image_save(image, dir=dir, prefix=filename)

    def pin_detection(self, gripper_type, ref):
        flex_logger.info("Pin detection")
        ...
        if int(gripper_type) == 1:
            ...
            flex_logger.info("Pin is in the gripper %s" % str(PinIsInGripper))
            if PinIsInGripper:
            ...
            flex_logger.info("Vertical edge of the pin %s" % str(edge))
            if edge is not None:
        ...
        if int(gripper_type) == 3:
            if ref == "True":
                ...
                self.save_ref_image(image, filename)
            flex_logger.info("gripper for pin detection is %s" % gripper_type)
            ...
            if PinIsInGripper:
                flex_logger.info("Pin is in gripper")
                self.robot.setVal3GlobalVariableBoolean("bPinIsInGripper", True)
                edge = self.cam.horizontal_edge(image, roi_pin)
                flex_logger.info("Horizontal edge of the pin %s" % str(edge))
                if edge is not None:
                    ...
                    if abs(distance_from_ref) <= self.get_detection_param(
                        ...
                        flex_logger.error("distance from reference is too high")
                ...
                if edge_gripper is not None or edge_pin is not None:
                    ...
                    if (
                        ...
                        flex_logger.info("distance saved in robot")
                    else:
                        flex_logger.error(
                    ...
                    flex_logger.error("Edge of the gripper or of the pin not found")
                    raise RuntimeError("Edge of the gripper or of the pin not found")
            else:
                flex_logger.info("Pin is not in gripper, calling vial detection")
    ...
    def vial_center_detection(self):
        ...
        flex_logger.info("center %s" % str(center))
        return center
    ...
    def vial_centering(self, center_1, center_2, center_3):
        ...
        if center_1 is None or center_2 is None or center_3 is None:
            flex_logger.error("Correction not applicable")
        ...
        if abs(gripper_Xoffset) < 3 or abs(gripper_Yoffset) < 3:
            ...
            flex_logger.error("Correction is too high")
    ...
    def vial_detection(self, image):
        ...
        if VialIsNotInGripper:
            ...
            flex_logger.info("Ref pin edge position %s" % str(ref_image_edge))
            cap_height = 3.0
            ref_vial_edge = cap_height * self.cam.pixels_per_mm + ref_image_edge
            flex_logger.info("Reference vial edge position %s" % str(ref_vial_edge))
            if abs(ref_vial_edge - vial_edge) < 200:
                self.robot.setVal3GlobalVariableBoolean("bVialIsInGrip", True)
                flex_logger.info("Vial is in gripper")
            else:
                flex_logger.info("Vial edge too different to the reference")
                self.robot.setVal3GlobalVariableBoolean("bVialIsInGrip", False)
    ...
    def dm_detection(self, GripperType):
        ...
        if int(GripperType) == 3:
            try:
                ...
                dm = None
        flex_logger.info("Data matrix is %s" % dm)
            ...
            while True:
                ...
                if notify == "PinDetection":
                    ...
                    if run != str(len(centers)):
                        flex_logger.error("image lost in vial detection")
                        # raise RuntimeError("image lost in vial detection")
                    if len(centers) == 3:
                        flex_logger.info("Calling Vial centering")
                        self.vial_centering(*centers)
                else:
                    flex_logger.info("detection loop")
        finally:
            if dm_reading:
                dm_reading.kill()
    ...
    def update_transfer_iteration(self, reset=False):
        ...
        with open(file_path, "wb") as file:
            parser.write(file)
        flex_logger.info("number of sample transfer set to %d" % (int(iter_nb)))
        return iter_nb
    ...
    def save_loaded_position(self, cell, puck, sample):
        ...
        with open(file_path, "wb") as file:
            parser.write(file)
        flex_logger.info("loaded position written (%d, %d, %d)" % (cell, puck, sample))
    ...
    def check_gripper_type(self, cell):
        ...
        if gripper_type in [1, 3]:
            ...
            if (gripper_type == 1 and not unipuck_cell) or (
                ...
                flex_logger.error("gripper/puck mismatch")
    ...
    def loadSample(self, cell, puck, sample, ref=False):
        ...
        if self.robot.getCachedVariable("data:dioPinOnGonio").getValue() == "true":
            flex_logger.error("Sample already on SmartMagnet")
    ...
    def unloadSample(self, cell, puck, sample):
        ...
        if self.robot.getCachedVariable("data:dioPinOnGonio").getValue() == "false":
            flex_logger.error("No sample on SmartMagnet")
    ...
    def chainedUnldLd(self, unload, load):
        ...
        if not isinstance(unload, list) or not isinstance(load, list):
            flex_logger.error("unload/load pos must be list")
        ...
        if unload_PuckType != load_PuckType:
            flex_logger.error("unload and load puck types must be identical")
    ...
    def trashSample(self):
        ...
        gripper_type = self.get_gripper_type()
        flex_logger.info("Starting trash sample")
        if gripper_type == 3:
            self.homeClear()
            self.poseGripper()
            ...
            flex_logger.info("Trash sample available only with Flipping gripper")

        if self.pin_on_gonio() == False:
            self._loaded_sample = -1, -1, -1
            self.save_loaded_position(*self._loaded_sample)
        flex_logger.info("Trash sample finished")

    def sampleStatus(self, status_name):
        while True:
            notify = self.robot.waitNotify(status_name)
            flex_logger.info("From Robot: %s %s" % (status_name, notify))
    ...
    def poseGripper(self):
        self.homeClear()
        flex_logger.info("Putting the gripper back in tool bank")
        self.onewire.close()
        self.do_poseGripper()
        self.robot.setVal3GlobalVariableDouble("nGripperType", "-1")
        flex_logger.info("Gripper back on tool bank")
    ...
    def takeGripper(self, gripper_to_take, defreeze=True):
        flex_logger.info("Starting to take gripper on tool bank")
        self.onewire = OneWire(self.ow_port)
        all_unipucks = len(self.unipuck_cells) == 8
        if (gripper_to_take not in [1, 3, 9]) or (
            ...
            raise RuntimeError("No or wrong gripper")
        gripper_type = self.get_gripper_type()
        flex_logger.info("Gripper is %s" % str(gripper_type))
        if gripper_type not in [-1, 0, 1, 3, 9]:
            flex_logger.error("wrong gripper on arm")
            raise RuntimeError("wrong gripper on arm")
        if gripper_type == -1:
            self.robot.setVal3GlobalVariableBoolean("bGripperIsOnArm", False)
            if gripper_to_take != 1 and gripper_to_take != 3 and gripper_to_take != 9:
                flex_logger.error("Wrong gripper")
            ...
            self.robot.setVal3GlobalVariableBoolean("bGripperIsOnArm", True)
            self.robot.setVal3GlobalVariableDouble("nGripperType", str(gripper_type))
        flex_logger.info("Gripper on robot")
        flex_logger.info("Starting defreezing gripper if needed")
        if defreeze == True:
            self.do_defreezeGripper()
        self.update_transfer_iteration(reset=True)
        flex_logger.info("Defreezing gripper finished")
    ...
    def changeGripper(self, gripper_to_take=1, user_mode=True):
        gripper_type = self.get_gripper_type()
        if gripper_type in [1, 3, 9]:
            flex_logger.info("first pose gripper %d" % gripper_type)
            self.poseGripper()
            if user_mode == False:
                ...
                if gripper_type == 1:
                    ...
                    flex_logger.error("gripper left unknown")
                    raise RuntimeError("gripper left unknown")
        elif gripper_type == -1:
            flex_logger.info("No gripper on arm taking gripper %d" % gripper_to_take)
            self.takeGripper(int(gripper_to_take))
        else:
            flex_logger.error("Wrong gripper on arm")
            raise RuntimeError("Wrong gripper on arm")
    ...
    def yag_pos(self):
        ...
        flex_logger.info("YAG focus and rotation: %s, %s" % (str(x), str(rz)))
        return x, rz

    def yag_reset(self):
        flex_logger.info("reset Yag positions")
    ...
    def yag_in(self):
        flex_logger.info("Starting YAG in")
        if self.robot.getCachedVariable("data:dioPinOnGonio").getValue() == "true":
            flex_logger.error("Sample on SmartMagnet")
            raise RuntimeError("Sample on SmartMagnet")
        self.yag_reset()
        gripper_type = self.get_gripper_type()
        if gripper_type == 1:
            flex_logger.info("Change gripper")
            ...
            flex_logger.info("Please change gripper")
            return
        flex_logger.info("YAG in done")
    ...
    def yag_out(self):
        flex_logger.info("Starting YAG out")
        self.do_yag_out()
        if self.swap_gripper is True:
            self.changeGripper()
        self.yag_reset()
        flex_logger.info("YAG out done")
    ...
    def yag_focus(self, focus=0):
        ...
        if start_focus == focus:
            flex_logger.info("No need to move")
            return
        if (focus < -5) or (focus > 5):
            flex_logger.error("focus must be in range -5 and +5 mm")
    ...
    def yag_rot(self, angle=0):
        ...
        if (angle < -90) or (angle > 90):
            flex_logger.error("angle must be in range -90 and +90 degrees")
    ...
    def spine_gripper_centering(
        ...
        if abs(gripper_Xoffset) < 1 or abs(gripper_Yoffset) < 1:
            ...
            flex_logger.error("gripper offsets are wrong")
    ...
    def flipping_gripper_height_detections(self):
        ...
        flex_logger.info("height to middle of the image %s" % str(height_dewar))
        self.robot.execute("data:pTemp = here(flange,world)")
        flipping_z_robot = self.robot.getVal3GlobalVariableDouble("pTemp.trsf.z")
        flex_logger.info("from robot z is %s" % str(flipping_z_robot))
        calib_z_robot = self.config.getfloat("Calibration", "z")
        flex_logger.info("from reference %s" % str(calib_z_robot))
        ...
        flex_logger.info("height to the middle of the image %s" % str(height_gonio))
        self.robot.execute("data:pTemp = here(flange,world)")
        flipping_z_robot = self.robot.getVal3GlobalVariableDouble("pTemp.trsf.z")
        flex_logger.info("from robot z is %s" % str(flipping_z_robot))
        flex_logger.info("from reference %s" % str(calib_z_robot))
        ...
        flex_logger.info("error in Z at the gonio %s" % str(diff_calib_flipping))
        ...
        if abs(height_dewar) < 5 and abs(height_gonio) < 5:
            ...
            flex_logger.error("Vertical correction too high")
    ...
    def stallion_centering(self, center1, center2, center3):
        ...
        if abs(stallion_Xoffset) < 2 and abs(stallion_Yoffset) < 2:
            if (
                ...
                flex_logger.info("X,Y calibration done for DW orientation")
                self.robot.execute(
                ...
                flex_logger.info("X,Y calibration done for gonio orientation")
                self.robot.execute(
    ...
    def ball_center_detection(self):
        ...
        if abs(rad1 - rad2) > 20:
            flex_logger.error("ellipse radii are too different")
    ...
    def save_translation(self):
        flex_logger.info("Starting save translation")
        tCalib = str(self.robot.getVal3GlobalVariableDouble("pTemp.trsf.z"))
        flex_logger.info("Translation calibration from robot %s" % str(tCalib))
        if tCalib == "110.0":
            flex_logger.error("problem with getVal3GlobalVariableDouble")
        ...
        try:
            ...
            flex_logger.info("No such file %s" % file_path)
        self.config.set("Calibration", "z", str(tCalib))
        with open(file_path, "wb") as file:
            self.config.write(file)
        flex_logger.info("file written")
        self.robot.setVal3GlobalVariableBoolean("bImageProcEnded", True)
    ...
    def calib_detection(self, gripper_type):
        ...
        while True:
            notify = self.robot.waitNotify("ImageProcessing")
            flex_logger.info("notify %s" % notify)
            if (
                notify == "TakeFlippingGripperCalibrationStallion_Z"
                and int(gripper_type) == 3
            ):
                flex_logger.info("calculation of height (Dewar and Gonio positions)")
                self.flipping_gripper_height_detections()
            if notify.startswith("GripperCalibration"):
                if int(gripper_type) == 1:
                    flex_logger.info("start gripper center detection")
                    centers.append(self.spine_gripper_center_detection())
                    flex_logger.info("number of items in center %d" % len(centers))
                if int(gripper_type) == 3:
            ...
            if notify == "GetTranslation" and int(gripper_type) == 9:
                flex_logger.info("Get translation")
                self.save_translation()
            if int(gripper_type) == 1 and len(centers) == 3:
                flex_logger.info("calculating 3-click centering with %s" % str(centers))
                centers_list = itertools.chain(*centers)
                flex_logger.info("center list %s" % str(centers_list))
            ...
            if int(gripper_type) == 9 and len(centers) == 3:
                flex_logger.info("calculating 3-click centering")
                centers_list = itertools.chain(*centers)
                flex_logger.info("center list %s" % str(centers_list))
    ...
    def gripperCalib(self):
        flex_logger.info("Starting calibration tool")
        gripper_type = self.get_gripper_type()
        if gripper_type in [1, 3, 9]:
            self.robot.setVal3GlobalVariableDouble("nGripperType", str(gripper_type))
            self.set_cam(gripper_type)
    ...
    def gonioAlignment(self):
        flex_logger.info("Starting calibration of the SmartMagnet position")
        gripper_type = self.get_gripper_type()
        if gripper_type != 9:
            flex_logger.error("Need calibration gripper")
            raise RuntimeError("Need calibration gripper")
        self.set_io("dioUnloadStReq", True)
        self.do_gonioAlign(200)
        flex_logger.info("calibration of the SmartMagnet position finished")
        self.set_io("dioUnloadStReq", False)
    ...
    def dewarAlignment(self, cell="all"):
        flex_logger.info("Starting calibration of the Dewar position")
        gripper_type = self.get_gripper_type()
        if gripper_type != 9:
            flex_logger.error("Need calibration gripper")
            raise RuntimeError("Need calibration gripper")
        ...
        if cell == "all":
            ...
            flex_logger.error("Wrong cell (default all or 1-8)")
            raise RuntimeError("Wrong cell (default all or 1-8)")
        flex_logger.info("calibration of the Dewar position finished")

    def get_phases(self, cell, frequency):
        flex_logger.info("Get phases on cell %d at %s Hz" % (cell, str(frequency)))
    ...
    def find_ref(self, frequency, cell):
        ...
        for list in ref:
            if list[0] == section:
                rank = i
                # flex_logger.info("Found the reference in %s" %str(list[0]))
                break
            if list == ref[-1:]:
                flex_logger.error("Reference not found")
    ...
    def detect_puck_type(self, cell):
        ...
        if len(ref_phase_800) != len(ref_phase_2000):
            flex_logger.error("config files have different length")
        ...
        for puckType in range(0, len(ref_phase_800)):
            ...
            for i in range(0, 3):
                ...
                if (
                    ...
                    # flex_logger.info("list append with %s" %str(ref_puckType[puckType]))
                    list_res.append(ref_puckType[puckType])
                else:
                    # flex_logger.info("list append with None")
                    list_res.append(None)
            result.append(list_res)
        # flex_logger.info("list of detection before filtering %s" %str(result))
    ...
    def _scanSlot(self, cell):
        ...
        flex_logger.info("threshold (us) %s" % str(threshold_us))
        for i in range(0, 3):
            if pucks_detected[i] == "empty":
                # threshold_puck1 = min(phase_puck1 + ref_threshold, ref_empty_puck1 - 0.5)
                threshold = phases_2000[i] - threshold_us
                self.proxisense.writeThreshold(cell, i + 1, threshold)
                # flex_logger.info("Puck not detected")
            else:
                threshold = phases_2000[i] + threshold_us
                self.proxisense.writeThreshold(cell, i + 1, threshold)
                # flex_logger.info("Puck detected")
            flex_logger.info("Puck%d threshold %s" % ((i + 1), str(threshold)))
            self.proxisense.writeThreshold(cell, i + 1, threshold)
    ...
    def scanSlot(self, cell="all"):
        if cell != "all" and isinstance(cell, int) and not cell in range(1, 9):
            flex_logger.error("Wrong cell number [1-8]")
            raise ValueError("Wrong cell number [1-8]")
        flex_logger.info("Starting to scan on cell %s" % (str(cell)))
        ...
        for cell in range(cell_start, cell_stop):
            flex_logger.info("Scan slot cell %d" % cell)
            self._scanSlot(cell)
