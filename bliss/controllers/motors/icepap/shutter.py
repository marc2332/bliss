...
class Shutter(BaseShutter):
    ...
    def axis(self):
        if self.mode == self.CONFIGURATION:
            ...
            raise RuntimeError("Shutter %s is not in configuration mode, "
                               " please switch to that mode before asking "
                               " the reference axis" % self.name)

    @property
    def closed_position(self):
        return self.settings.get('closed_position',
                                 self.config.get('closed_position'))
    @closed_position.setter
    def closed_position(self,position):
        if self.mode != self.CONFIGURATION:
            raise RuntimeError("Shutter %s: can set the closed position, "
                               "not in Configuration mode" % self.name)
        self.settings.set('closed_position',position)

    @property
    def opened_position(self):
        return self.settings.get('opened_position',
                                 self.config.get('opened_position'))
    @opened_position.setter
    def opened_position(self,position):
        if self.mode != self.CONFIGURATION:
            raise RuntimeError("Shutter %s: can set the opened position, "
                               "not in Configuration mode" % self.name)
    ...
    def __init__(self,name,controller,config):
        ...
        self._axis = None
        self._position_loaded = Cache(self,"loaded-position",
    ...
    def _set_mode(self,mode):
        ...
        if mode == self.EXTERNAL:
            ...
            opened_position = self.settings.get('opened_position',
                                                self.config.get('opened_position'))
