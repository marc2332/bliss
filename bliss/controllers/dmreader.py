...
class DmReader(object):
   ...
   def connect(self, timeout=20):
       ...
       try:
         ...
         raise RuntimeError("cannot connect to datamatrix reader (%s, %d)" % (self.controller_ip, self.port))
       else:
         return s
   ...
   def get_list(self):
       ...
       if s.recv(3) != "TA\r":
         raise RuntimeError("Could not retrieve data matrices from Keyence controller")
   ...
   def wrt_exec_condition(self,condition):
     ...
     if s.recv(3) == "ER\r":
       ...
       while True:
         ...
         if data_list.endswith("\r"):
           if data_list[:-1].split(",")[-1] == "22":
             raise RuntimeError("The number or the range of the parameter is incorrect")
     ...
     with gevent.Timeout(10, RuntimeError("no sample detected")):
       ...
       for i in range(3):
         ...
         if bool(1-int(all_presence[(puck_number-1)*11])):
           res = (True, bool(1-int(all_presence[sample_number+(puck_number-1)*11])))
         else:
           res = (False, False)
