...
def fwdKin(leg1, leg2, leg3):
    ...
    u = numpy.array([L1.u, L2.u, L3.u], dtype=numpy.float) # get the z positions of the jacks
    ...
    for k in range(100):
        J = numpy.array([[2*a1 + 2 * x[0,0], 2*a2 + 2*x[1,0]],
                     [2*b1 + 2 * x[0,0], 2*b2 + 2*x[1,0]]], dtype=numpy.float)
        F = numpy.array([(a1 + x[0, 0])**2 + (a2 + x[1, 0])**2 + a3,
                         (b1 + x[0, 0])**2 + (b2 + x[1, 0])**2 + b3],
                         dtype=numpy.float).reshape(2,1)
    ...
    if ((dab[2] == 0) and (dac[2] == 0) and (dbc[2] == 0)):
        ...
        U = concatenate((\
                concatenate((eye(3)*dab[0], eye(3)*dab[1], eye(3)*dab[2]),
                            axis=1),
                concatenate((eye(3)*dac[0], eye(3)*dac[1], eye(3)*dac[2]),
                            axis=1),
                concatenate((eye(3)*dbc[0], eye(3)*dbc[1], eye(3)*dbc[2]),
                            axis=1)), axis=0)
    ...
    #print("rz = ", rz)
    #print("rx = ", rx)
    csry = numpy.array([[cos(rz), sin(rx) * sin(rz)],
                        [sin(rx) * sin(rz), -cos(rz)]], dtype=numpy.float)
    csry = numpy.dot(numpy.linalg.inv(csry),
                     numpy.concatenate((R[0:1], R[2:3]), axis=0))
    ...
    Hs = numpy.dot(htrans(l1.w), \
         numpy.dot(euler2H(rx,ry,0), \
         numpy.dot(euler2H(0,0,rz), htrans(-l1.s))))
    z = Hs[2,3] + u[0]
