...
class HexapodProtocolV2(BaseHexapodProtocol):
    ...
    SYSTEM_STATUS_FIELDS = ('error', 'ready', 'emergency_stop_pressed',
                            'safety_enabled', 'in_position', 'moving',
                            'control', 'brake', 'phasing_done', 'homing_done',
                            'homing_running', 'homing_virtual',
                            'motion_restrained', 'software_limit',
                            'following_error', 'hardware_limit', 'drive_fault',
                            'encoder_error', 'system_error', 'fatal_error')

    SystemStatus = namedtuple('SystemStatus', SYSTEM_STATUS_FIELDS)

    AXIS_STATUS_FIELDS = ('error', 'in_position', 'moving', 'control',
                          'brake', 'phasing_done', 'homing_done',
                          'homing_running', 'homing_hardware_input',
                          'software_limit', 'following_warning',
                          'following_error', 'negative_hardware_limit',
                          'positive_hardware_limit', 'drive_fault',
                          'encoder_error', 'fatal_error')
...
"""
    ...
    def __handle_reply_code(cls, cmd, reply, expected_code):
        ...
        if not cmd.startswith(reply_cmd):
            raise HexapodV2Error('Reply command ({0!r}) does not match '
                                 'expected command ({1!r}'.format(reply_cmd, cmd))
        try:
            reply_dict['code'] = reply_code = int(reply_dict['code'])
        except ValueError:
            raise HexapodV2Error('Unexpected reply code {0!r}'
                                 .format(reply_code))
    ...
    def __call__(self, cmd, *args, **kwargs):
        ...
        if kwargs:
            raise TypeError('Unknown keyword arguments: {0}'
                            .format(', '.join(kwargs)))
        is_query = '?' in cmd
    ...
    def __to_status(self, status, klass):
        istatus = int(status)
        return klass(*[bool(istatus & (1 << i))
                       for i in range(len(klass._fields))])
    ...
    def speeds(self, speeds):
        ...
        if 'tspeed' not in speeds:
            ...
            raise HexapodV2Error('Translational speed above max allowed ({0} mm/s)'
                                 .format(curr_speeds['max_tspeed']))
        if 'rspeed' not in speeds:
            speeds['rspeed'] = self.rspeed
        elif speeds['rspeed'] > curr_speeds['max_rspeed']:
            raise HexapodV2Error('Rotational speed above max allowed ({0} deg/s)'
                                 .format(curr_speeds['max_rspeed']))
    ...
    def accelerations(self, accels):
        ...
        if 'tacceleration' not in accels:
            ...
            raise HexapodV2Error('Translational acceleration above max allowed ({0} mm/s/s)'
                                 .format(curr_accels['max_tacceleration']))
        if 'racceleration' not in accels:
            ...
            raise HexapodV2Error('Rotational acceleration above max allowed ({0} deg/s/s)'
                                 .format(curr_accels['max_racceleration']))

        self('CFG#ACC', self.__from_tr(accels['tacceleration'], accels['racceleration']))
    ...
    def specific_poses(self):
        ...
        for i in range(64):
            if pose_bits & (1<<i):
                ...
                poses[i+1] = self.SpecificPose(id=i+1,
    ...
    def __from_pose(cls, pose):
        data = numpy.array(pose, dtype=float)
    ...
    def dump(self):
        ...
        rows = [['User coordinate system'] + list(user_cs),
        ...
        rows = [list(specific_poses[idx][:-1]) + list(specific_poses[idx][-1])
        ...
        return self.DUMP_TEMPLATE.format(o=self, speeds=speeds,
...
class SHexapod(Controller):
    ...
    def protocol(self):
        ...
        for klass in (HexapodProtocolV2, HexapodProtocolV1):
            try:
