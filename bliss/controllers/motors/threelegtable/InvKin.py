...
def invKin(leg1, leg2, leg3, sample):
    ...
    if (l2.fixDof[0] == 0) and (l2.fixDof[1] == 1):
        ...
        # use newton methode to find the solution
        for k in range(100):
            J = sx*sy*(ds[0]*cos(rz) - ds[1]*sin(rz)) - cy * (ds[1]*cos(rz) + ds[0]*sx)
            F = dw[0] + cy*(ds[0]*cos(rz) - ds[1]*sin(rz)) + cx*ds[2]*sy + \
                sx*sy*(ds[1]*cos(rz) + ds[0]*sin(rz))
            rz = rz - F/J
    
    else: 
        raise ValueError('Error one actuator has not only one degree of freedom in x or y direction')
