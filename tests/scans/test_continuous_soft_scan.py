...
class DebugMotorMockupAcquisitionDevice(AcquisitionDevice):
    ...
    def start(self):
        event.connect(
            self.motor_mockup, 'internal_state', self.set_time_ref)
...
def test_software_position_trigger_master(beacon):
    ...
    assert data['roby'] == pytest.approx(data['debug_pos'], abs=0.05)
    assert data['debug_time'] == pytest.approx(expected_triggers, abs=0.005)
