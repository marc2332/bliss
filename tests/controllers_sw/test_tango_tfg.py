...
def test_tango_tfg_timing_info(tfg):
    ...
    timing_info = {
            'cycles': 2,
            'framesets': [{'nb_frames': 3, 'latency': 1e-07, 'acq_time': 0.1},
    ...
    timing_info = {
            'cycles': 1,
            'framesets': [{'nb_frames': 4, 'latency': 1e-07, 'acq_time': 0.1},
...
def test_tango_tfg_scalers(tfg):
    ...
    tfg._control.setupCCChan.assert_called_once_with([tfg.ScalerOptions['count_rising_edges'], tfg.ALL_CHAN])
    ...
    tfg._control.setupCCChan.assert_called_with([tfg.ScalerOptions['count_while_input_low'], tfg.ScalerInput['ScalIn5']])
