...
def wm(*axes, **kwargs):
    ...
    for axis in global_map.get_axis_objects_iter(*axes):
        ...
        User.append(None)
        high, highd = (
            (high, axis.user2dial(high))
            if not high in (None, err)
            else (_MISSING_VAL, _MISSING_VAL)
        )
        low, lowd = (
            (low, axis.user2dial(low))
            if not low in (None, err)
            else (_MISSING_VAL, _MISSING_VAL)
        )
        high_user.append(high)
        position = get(axis, "position")
        user.append(position)
        low_user.append(low)
        Dial.append(None)
        high_dial.append(highd)
        dial_position = get(axis, "dial")
        dial.append(dial_position)
        low_dial.append(lowd)
