...
key as the counter name and a numpy array with the assaociated data.
    ...
    Scan 3 Wed Mar 06 11:47:53 2019 /tmp/scans/test_session/data.h5 test_session user = seb
    loopscan 2 0.1
...
### Example
    ...
    Scan 7 Wed Mar 06 12:45:56 2019 /tmp/scans/test_session/data.h5 test_session user = seb
...
### Scan example
    CYRIL [14]: s = timescan(1, sim_acq_dev.counters.sc1, run=False) 

    CYRIL [15]: s.trace(True)
...
### ct example
    ...
    CYRIL [4]: s = ct(1, sim_acq_dev.counters.sc1,run=False)
    
    CYRIL [6]: s.trace(True);s.run()
