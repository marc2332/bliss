...
GENERAL_PARAMS_SEQ = (
    ...
    (
        "address",
        ("str", None, True, None, "Address", None, "card address (ex: /dev/p201)"),
    ),
    (
        "clock",
        (
            ...
            None,
            True,
            card.Clock.CLK_100_MHz,
            "Clock",
            None,
            "counters external clock",
        ),
    ),
    (
        "dma interrupt",
        (
            ...
            "DMA interrupt",
            "interrupt",
            "End of DMA transfer will trigger interrupt",
        ),
    ),
    (
        ...
        (
            ...
            "FIFO half full interrupt",
            "interrupt",
            "FIFO half full will trigger interrupt",
        ),
    ),
    (
        "error interrupt",
        (
            ...
            "Error interrupt",
            "interrupt",
            "FIFO transfer error or too close DMA triggers will trigger interrupt",
        ),
    ),
)
...
COUNTER_PARAMS_SEQ = (
    ...
    (
        ...
        (
            ...
            None,
            True,
            card.CtHardStartSrc.SOFTWARE,
            "Start",
            "source",
            "hardware start source",
        ),
    ),
    (
        "stop source",
        (
            ...
            None,
            True,
            card.CtHardStopSrc.SOFTWARE,
            "Stop",
            "source",
            "hardware stop source",
        ),
    ),
    (
        "latch sources",
        (
            ...
            "Latch(es)",
            "source",
            "counter(s) to latch on counter signals hardware stop, software stop and software disable",
        ),
    ),
    (
        "reset",
        (
            ...
            "Reset",
            None,
            "reset from hardware or software stop",
        ),
    ),
    ...
    (
        ...
        (
            ...
            "Interrupt",
            "interrupt",
            "counter N stop triggers interrupt",
        ),
    ),
    (
        "latch triggers dma",
        (
            ...
            "Latch triggers DMA",
            "interrupt",
            "DMA is triggered on counter N latch signal",
        ),
    ),
    (
        "fifo on dma trigger",
        (
            ...
            "FIFO on DMA trigger",
            "interrupt",
            "Latch N value stored to FIFO when DMA triggered",
        ),
    ),
    (
        "software enable",
        ("bool", None, True, False, "Soft. enable", None, "software enable/disable"),
    ),
...
IN_CHANNEL_PARAMS_SEQ = (
    ...
    (
        ...
        (
            ...
            "Level",
            None,
            "channel level (TTL, NIM, Auto or None)",
        ),
    ),
    ...
    (
        ...
        (
            ...
            "Edge interrupt",
            None,
            "edge interrupt: rising, falling or both (comma separated)",
        ),
    ),
)
...
OUT_CHANNEL_PARAMS_SEQ = (
    ...
    (
        "software enable",
        ("bool", None, True, False, "Sw. enable", None, "software enable"),
    ),
    (
        "level",
        (
...
def get_tree_files(cfg):
    ...
    if klass is None:
        result = dict(
            type="ct/ch",
            path=os.path.join(get_tree_files(cfg.parent)["path"], cfg["name"]),
            icon="fa fa-square",
        )
    else:
        result = dict(
...
def get_tree_items(cfg):
    ...
    if klass is None:
        result = dict(
            type="ct/ch",
            path=os.path.join(get_tree_items(cfg.parent)["path"], cfg["name"]),
            icon="fa fa-square",
        )
    else:
        result = dict(type="Counter card", path=cfg["name"], icon="fa fa-credit-card")
    return result
...
def get_card_html(cfg):
    ...
    counters = dict([(i, dict(address=i)) for i in card_class.COUNTERS])
    for ct in cts:
        counters[ct["address"]] = ct
    channels = cfg.setdefault("channels", [])
    in_channels = dict([(i, dict(address=i)) for i in card_class.INPUT_CHANNELS])
    out_channels = dict([(i, dict(address=i)) for i in card_class.OUTPUT_CHANNELS])
    for ch in channels:
        addr = ch["address"]
