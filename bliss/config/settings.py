...
class SimpleSetting(object) :
    class Proxy(object) :
        def __init__(self,name,cnx,
                     read_type_conversion,write_type_conversion) :
            self._cnx = weakref.ref(cnx)
            self._name = name
    ...
    def __get__(self,obj,type = None) :
        if self._use_object_name:
            ...
            name = self._name
        return SimpleSetting.Proxy(name,self._cnx,
...
class QueueSetting(object):
    ...
    def __get__(self,obj,type = None) :
        ...
        return QueueSetting.Proxy(name,self._cnx,
                                  self._read_type_conversion,
                                  self._write_type_conversion)
    ...
    def __set__(self,obj,values) :
        ...
        proxy = QueueSetting.Proxy(name,self._cnx,
                                   self._read_type_conversion,
                                   self._write_type_conversion)
        proxy.set(values)
...
class HashObjSetting(HashSetting) :
    def __init__(self,name,connection = None,**keys) :
        HashSetting.__init__(self,name,connection=connection,
                             read_type_conversion=pickle.loads,
                             write_type_conversion=pickle.dumps,**keys)
class QueueObjSetting(QueueSetting) :
    def __init__(self,name,connection = None,**keys) :
        QueueSetting.__init__(self,name,connection=connection,
                              read_type_conversion=pickle.loads,
                              write_type_conversion=pickle.dumps,**keys)
class SimpleObjSetting(SimpleSetting) :
    def __init__(self,name,connection = None,**keys) :
        SimpleSetting.__init__(self,name,connection=connection,
                               read_type_conversion=picke_loads,
                               write_type_conversion=pickle.dumps,**keys)

class Struct(object):
    def __init__(self,name,**keys) :
        self._proxy = HashSetting(name,**keys).get_proxy()
...
if __name__ == "__main__" :
    class A(object) :
        x = SimpleSetting('counter')
        y = SimpleObjSetting('obj')
        q = QueueSetting('seb')
        ol = QueueObjSetting('seb-list')
        h = HashSetting('seb-hash')
        oh = HashObjSetting('seb-hash-object')
