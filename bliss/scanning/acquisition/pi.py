...
class PIAcquisitionDevice(AcquisitionDevice):
    ...
    def set_counters(self, *counters_params):
        ...
        if nb_counters * 3 != len(counters_params):
            raise RuntimeError("Arguments must be grouped by 3 "
                               "(counter_name,motor,recorder_type...)")

        self.channels[:] = [AcquisitionChannel('timestamp',
                                               numpy.double, ())]
        self.__motor_data_type = list()
        for counter_name, motor, recorder_type in grouped(counters_params, 3):
            self.channels.append(AcquisitionChannel(counter_name,
                                                    numpy.double, ()))
    ...
    def trigger(self):
        ...
        self.device.start_recording(trigger_source,
                                    value=self.external_input,
                                    recorder_rate=self.count_time)
        self.__started = True
        self.__event.set()
    ...
    def reading(self):
        ...
        while self._nb_acq_points < self.npoints and \
              not self.__stopped:
            data = self.device.get_data(self._nb_acq_points,
                                        npoints=self.npoints - self._nb_acq_points)
            if data is not None:
