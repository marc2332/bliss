...
class MD2M:
    ...
    def _wait_ready(self, *axes, **kwargs):
        ...
        with gevent.Timeout(timeout, RuntimeError("Timed out waiting for motors to be READY")):
            for axis in axes:
                axis.wait_move()
    ...
    def _helical_calc(self, helical_pos, exp_time):
      ...
      helical = { "phiy": {"trajectory": end.get("phiy", 0)-start.get('phiy', 0), "motor": self.phiy, "start":start.get("phiy") },
    ...
    def oscil(self, start_ang, stop_ang, exp_time, save_diagnostic=True, operate_shutter=True, helical=False):
      d, calc_velocity, acc_time, acc_ang = self._oscil_calc(start_ang, stop_ang, exp_time)

      def oscil_cleanup(v=self.omega.velocity(from_config=True), a=self.omega.acceleration(from_config=True)):
    ...
    def save_diagnostic(self, phi_encoder_pos=0):
        ...
        with open(self.diagfile, "a+") as diagfile:
            ...
            diagfile.write("#N %d\n" % cols) 
            diagfile.write("#L Time(ms)  Speed  Phi  PhiY  I1  Shut Cmd  Shut State  Detector Acquiring  PhiZ\n")
    ...
    def centrebeam(self):
        ...
        for pos in self.zoom_positions:
            if abs(float(pos.get("offset")) - zoom_pos)<0.001:
                px_mm_y, px_mm_z = float(pos.get("pixelsPerMmY")) or 0, float(pos.get("pixelsPerMmZ")) or 0
                break
        else:
            raise RuntimeError("Could not get pixels/mm calibration")

        def restore_table(saved_pos=(self.tz1, self.tz1.position(), self.tz2, self.tz2.position(), self.tz3, self.tz3.position(), self.ttrans,self.ttrans.position())):
