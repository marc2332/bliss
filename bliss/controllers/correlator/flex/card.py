...
MODE = enum.Enum('MODE', 'SINGLE_AUTO SINGLE_CROSS '
                 'DUAL_AUTO DUAL_CROSS QUAD')
_MODE_STRUCT = namedtuple("AcqMode", "init start")

class Card(object):
    MODE = MODE
    ENUM_2_ACQ = {e:_MODE_STRUCT(ctypes.c_byte(ord(i)), s)
                  for e, (i, s) in zip(MODE, (('A', 0), ('A', 1),
                                              ('B', 0), ('B', 1),
                                              ('C', 0)))}
    ...
    def __init__(self, name, low_level_library="flex02-01d.dll"):
        self._name = name
        self._parameters = HashObjSetting('%s:parameters' % name,
                                          default_values={
                                              'mode':Card.MODE.SINGLE_AUTO})
    ...
    def _init_lib(self):
        if self._init is False:
            ...
            if not self._lib._USBInitialize(acq_struct.init):
                raise RuntimeError("Can not initialize correlator "
                                   " to mode **%s**" % str(mode))
            self._init = True
    ...
    def mode(self, mode):
        ...
        try:
            ...
            raise KeyError("Mode could be %s" %
                           [str(e) for e in self.MODE])
    ...
    def intensities_and_acqtime(self):
        ...
        """
        if self._traceA:
            intensityA = (float(sum(self._traceA)) /
                          (len(self._traceA) * self.FLEX_TIME))
        else:
            intensityA = 0.
        if self._traceB:
            intensityB = (float(sum(self._traceB)) /
                          (len(self._traceB) * self.FLEX_TIME))
        else:
            intensityB = 0.
    ...
    def trace(self):
        ...
        """
        traces = numpy.zeros(len(self._traceA),
                             dtype=[('time', 'd'),
                                    ('traceA', 'd'),
                                    ('traceB', 'd')])
    ...
    def data(self):
        ...
        """
        mode = self._parameters['mode']
        if mode in (self.MODE.SINGLE_AUTO,
                    self.MODE.SINGLE_CROSS):
        ...
        elif mode in (self.MODE.DUAL_AUTO,
                      self.MODE.DUAL_CROSS):
    ...
    def _calc_delay(linear_nb, nb_segment, nb_val_per_segment):
        ...
        for i in range(nb_segment):
            ...
            offset = delay[-1]
            sub_delay = offset + numpy.arange(1, nb_val_per_segment + 1,
                                              dtype=numpy.float) * flex_delay
    ...
    def _flex_read(card_proxy):
        ...
        while True:
            card_proxy._lib._USBUpdateRawdata(
                acq_time_p, trace_cnt_p, raw_corr_p, sample_p,
                baseA_p, baseB_p,
                traceA.ctypes.data, traceB.ctypes.data,
                histoA.ctypes.data, histoB.ctypes.data)
