...
class ImageRGB(object):
    ...
    def get_binned(self, ibin):
        ...
        for y in yblocks:
            ...
            for x in xblocks:
                ...
                t = gthreadpool.spawn(self.get_binned_quadrant,
                                      *(qd, (bin_x, bin_y), qr))
                tasks.append(t)
                x0 += x
            y0 += y
    ...
    def get_qtimage(self):
        return QtGui.QImage(self.data_aligned(), self.cols, self.rows,
...
class LeicaCamCtrlPacket(object):
    ...
    def __init__(self, ptype=None, pnb=None, addr=None, rlen=None, val=None,
                 rdata=None):
        self.ptype = ptype
        self.pnb = pnb
        self.addr = addr
    ...
    def getSrcCmd(klass, ptype):
        return [(x, y) for x in klass.TYPE
                       for y, p in klass.TYPE[x].items() if p == ptype][0]
    ...
    def frombytes(klass, s):
        ...
        if src == klass.HOST:
            ...
            rdata = data
        return klass(ptype=ptype, pnb=pnb, addr=addr, rlen=rlen, val=val,
                     rdata=rdata)
...
class LeicaCamImgPacketI(object):
    ...
    def tobytes(self):
        p = list(self.MAGIC[0])
        p += [0, 0, MAGIC[1], self.pnb, 0, MAGIC[2], MAGIC[3], self.tstamp,
              MAGIC[4], MAGIC[5], self.width, 0, self.height, 0, 0, 0]
        return struct.pack(self.HEADER, *p)

    def __str__(self):
        t = self.tstamp / self.CLOCK_FREQ
        s = 'pnb=%04x,tstamp=%.6f,width=%d,height=%d' % \
            (self.pnb, t, self.width, self.height)
        return '<%s>' % s
    
    @classmethod
    def frombytes(klass, s):
        d = struct.unpack(klass.HEADER, s)
        if (''.join(d[:5]) != klass.MAGIC[0] or
            d[5:8] != (0, 0, klass.MAGIC[1]) or d[9] != 0 or
            d[10:12] != klass.MAGIC[2:4] or d[13:15] != klass.MAGIC[4:6] or
            0 not in (d[16], d[18], d[19], d[20])):
            raise ValueError, 'Could not find signatures in header'
        pnb = d[8]
        tstamp = d[12]
        width = d[15]
...
class LeicaCamImgPacketII(object):
    ...
    def frombytes(klass, s):
        d = struct.unpack(klass.HEADER, s)
        if (''.join(d[:5]) != klass.MAGIC[0] or
            d[5:10] != (0, 0, klass.MAGIC[1], klass.MAGIC[2], 0) or
            0 not in (d[11], d[13])):
            raise ValueError, 'Could not find signatures in header'
        plen = d[10]
        height = d[12]
...
class LeicaCamera(LeicaUSB):
    ...
    def set_ctrl_data(self, addr, dval):
        ...
        if (ap.ptype != ptype or ap.pnb != self.pnb or
            struct.unpack('I', ap.rdata)[0] != 0x00040000):
            raise ValueError, 'Invalid ack packet: %s' % ap
        self.pnb += 1
