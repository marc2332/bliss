...
class _DataItem(_property_tree_helper.ScanRowItem):
    ...
    def __usedChanged(self, item: qt.QStandardItem):
        if self.__plotItem is not None:
            ...
            channel = plot_model.ChannelRef(plot, channelName)
            newItem.setYChannel(channel)
            # FIXME: Have to be properly setup with a concept of arange
            newItem.setXChannel(channel)
...
class OneDimPlotPropertyWidget(qt.QWidget):
    ...
    def __updateTree(self):
        ...
        for plotItem in self.__plotModel.items():
            ...
            if yChannelName in channelItems:
                channelItem = channelItems[yChannelName]
