...
class keithley428(object):
    ...
    PerformAutoSuppress  = _simple_cmd("C0N2X","Turn zero check off & perform auto suppress")
    ...
    def FilterRiseTime(self, value):
        if value not in self._FilterRiseTimes:
            raise ValueError, "Filter rise time value {0} out of range (0-9)".format(value)
    ...
    def CurrentSuppress(self, amps):
        ...
        if absVal > currentRangeMax:
            raise ValueError, "Current suppress value {0} out of range (=/-0.005A)".format(amps)
        currentRange = 0
        for x in range(7,0,-1):
            ...
            if absVal > currentRangeMax:
                ...
                break
        if currentRange == 0:
            raise ValueError, "Current suppress value {0} out of range (too small)".format(amps)
