...
class XPS:
    ...
    def ControllerMotionKernelMinMaxTimeLoadGet(self):
        command = ('ControllerMotionKernelMinMaxTimeLoadGet(' +
    ...
    def TCLScriptExecuteAndWait(self, TCLFileName, TaskName, InputParametersList):
        command = ('TCLScriptExecuteAndWait(' + TCLFileName + ',' + TaskName + ',' +
                   InputParametersList + ',char *)')
        return self.__sendAndReceive(command)

    # Execute a TCL script with defined priority
    def TCLScriptExecuteWithPriority(self, TCLFileName, TaskName, TaskPriorityLevel,
                                     ParametersList):
        command = ('TCLScriptExecuteWithPriority(' + TCLFileName + ',' + TaskName + ',' +
    ...
    def EventAdd(self, PositionerName, EventName, EventParameter, ActionName, ActionParameter1,
                 ActionParameter2, ActionParameter3):
        command = ('EventAdd(' + PositionerName + ',' + EventName + ',' + EventParameter +
                   ',' + ActionName + ',' + ActionParameter1 + ',' + ActionParameter2 +
                   ',' + ActionParameter3 + ')')
        return self.__sendAndReceive(command)
    ...
    def EventExtendedConfigurationTriggerSet(self, ExtendedEventName, EventParameter1,
    ...
    def EventExtendedConfigurationActionSet(self, ExtendedActionName, ActionParameter1,
    ...
    def GatheringDataMultipleLinesGet(self, IndexPoint, NumberOfLines):
        command = ('GatheringDataMultipleLinesGet(' + str(IndexPoint) + ',' +
    ...
    def GroupPositionCorrectedProfilerGet(self, GroupName, PositionX, PositionY):
        command = ('GroupPositionCorrectedProfilerGet(' + GroupName + ',' + str(PositionX) +
    ...
    def GroupPositionPCORawEncoderGet(self, GroupName, PositionX, PositionY):
        command = ('GroupPositionPCORawEncoderGet(' + GroupName + ',' + str(PositionX) +
                   ',' + str(PositionY) + ',double *,double *)')
        return self.__sendAndReceiveWithDecode(command)
    ...
    def GroupReferencingActionExecute(self, PositionerName, ReferencingAction, ReferencingSensor,
    ...
    def PositionerAnalogTrackingPositionParametersGet(self, PositionerName):
        command = ('PositionerAnalogTrackingPositionParametersGet(' + PositionerName +
                   ',char *,double *,double *,double *,double *)')
        return self.__sendAndReceiveWithDecode(command)

    # Update dynamic parameters for one axe of a group for a future analog tracking position
    def PositionerAnalogTrackingPositionParametersSet(self, PositionerName, GPIOName, Offset,
                                                      Scale, Velocity, Acceleration):
        command = ('PositionerAnalogTrackingPositionParametersSet(' + PositionerName + ',' +
                   GPIOName + ',' + str(Offset) + ',' + str(Scale) + ',' + str(Velocity) +
                   ',' + str(Acceleration) + ')')
        return self.__sendAndReceive(command)
    ...
    def PositionerAnalogTrackingVelocityParametersSet(self, PositionerName, GPIOName, Offset,
                                                      Scale, DeadBandThreshold, Order, Velocity,
                                                      Acceleration):
        command = ('PositionerAnalogTrackingVelocityParametersSet(' + PositionerName + ',' +
                   GPIOName + ',' + str(Offset) + ',' + str(Scale) + ',' +
                   str(DeadBandThreshold) + ',' + str(Order) + ',' + str(Velocity) + ',' +
                   str(Acceleration) + ')')
        return self.__sendAndReceive(command)
    ...
    def PositionerCompensatedPCOSet(self, PositionerName, Start, Stop, Distance, Width):
        command = ('PositionerCompensatedPCOSet(' + PositionerName + ',' + str(Start) + ',' +
                   str(Stop) + ',' + str(Distance) + ',' + str(Width) + ')')
        return self.__sendAndReceive(command)
    ...
    def PositionerCompensationFrequencyNotchsSet(self, PositionerName, NotchFrequency1,
                                                 NotchBandwidth1, NotchGain1, NotchFrequency2,
                                                 NotchBandwidth2, NotchGain2, NotchFrequency3,
                                                 NotchBandwidth3, NotchGain3):
        command = ('PositionerCompensationFrequencyNotchsSet(' + PositionerName + ',' +
                   str(NotchFrequency1) + ',' + str(NotchBandwidth1) + ',' + str(NotchGain1) +
                   ',' + str(NotchFrequency2) + ',' + str(NotchBandwidth2) + ',' +
                   str(NotchGain2) + ',' + str(NotchFrequency3) + ',' + str(NotchBandwidth3) +
                   ',' + str(NotchGain3) + ')')
        return self.__sendAndReceive(command)
    ...
    def PositionerCompensationLowPassTwoFilterSet(self, PositionerName, CutOffFrequency):
        command = ('PositionerCompensationLowPassTwoFilterSet(' + PositionerName + ',' +
                   str(CutOffFrequency) + ')')
        return self.__sendAndReceive(command)
    ...
    def PositionerCompensationNotchModeFiltersSet(self, PositionerName, NotchModeFr1, NotchModeFa1,
                                                  NotchModeZr1, NotchModeZa1, NotchModeFr2,
                                                  NotchModeFa2, NotchModeZr2, NotchModeZa2):
        command = ('PositionerCompensationNotchModeFiltersSet(' + PositionerName + ',' +
                   str(NotchModeFr1) + ',' + str(NotchModeFa1) + ',' + str(NotchModeZr1) +
                   ',' + str(NotchModeZa1) + ',' + str(NotchModeFr2) + ',' + str(NotchModeFa2) +
                   ',' + str(NotchModeZr2) + ',' + str(NotchModeZa2) + ')')
        return self.__sendAndReceive(command)
    ...
    def PositionerCompensationPhaseCorrectionFiltersSet(self, PositionerName, PhaseCorrectionFn1,
                                                        PhaseCorrectionFd1, PhaseCorrectionGain1,
                                                        PhaseCorrectionFn2, PhaseCorrectionFd2,
                                                        PhaseCorrectionGain2):
        command = ('PositionerCompensationPhaseCorrectionFiltersSet(' + PositionerName + ',' +
                   str(PhaseCorrectionFn1) + ',' + str(PhaseCorrectionFd1) + ',' +
                   str(PhaseCorrectionGain1) + ',' + str(PhaseCorrectionFn2) + ',' +
                   str(PhaseCorrectionFd2) + ',' + str(PhaseCorrectionGain2) + ')')
        return self.__sendAndReceive(command)
    ...
    def PositionerCompensationSpatialPeriodicNotchsSet(self, PositionerName, SpatialNotchStep1,
                                                       SpatialNotchBandwidth1, SpatialNotchGain1,
                                                       SpatialNotchStep2, SpatialNotchBandwidth2,
                                                       SpatialNotchGain2, SpatialNotchStep3,
                                                       SpatialNotchBandwidth3, SpatialNotchGain3):
        command = ('PositionerCompensationSpatialPeriodicNotchsSet(' + PositionerName + ',' +
                   str(SpatialNotchStep1) + ',' + str(SpatialNotchBandwidth1) + ',' +
                   str(SpatialNotchGain1) + ',' + str(SpatialNotchStep2) + ',' +
                   str(SpatialNotchBandwidth2) + ',' + str(SpatialNotchGain2) + ',' +
                   str(SpatialNotchStep3) + ',' + str(SpatialNotchBandwidth3) + ',' +
                   str(SpatialNotchGain3) + ')')
        return self.__sendAndReceive(command)

    # Update filters parameters
    def PositionerCorrectorNotchFiltersSet(self, PositionerName, NotchFrequency1, NotchBandwidth1,
                                           NotchGain1, NotchFrequency2, NotchBandwidth2, NotchGain2):
        command = ('PositionerCorrectorNotchFiltersSet(' + PositionerName + ',' +
                   str(NotchFrequency1) + ',' + str(NotchBandwidth1) + ',' + str(NotchGain1) +
                   ',' + str(NotchFrequency2) + ',' + str(NotchBandwidth2) + ',' +
                   str(NotchGain2) + ')')
        return self.__sendAndReceive(command)
    ...
    def PositionerCorrectorPIDBaseSet(self, PositionerName, MovingMass, StaticMass,
                                      Viscosity, Stiffness):
        command = ('PositionerCorrectorPIDBaseSet(' + PositionerName + ',' + str(MovingMass) +
                   ',' + str(StaticMass) + ',' + str(Viscosity) + ',' + str(Stiffness) + ')')
        return self.__sendAndReceive(command)
    ...
    def PositionerCorrectorPIDFFAccelerationSet(self, PositionerName, ClosedLoopStatus, KP, KI,
                                                KD, KS, IntegrationTime,
                                                DerivativeFilterCutOffFrequency, GKP, GKI, GKD,
                                                KForm, KFeedForwardAcceleration, KFeedForwardJerk):
        command = ('PositionerCorrectorPIDFFAccelerationSet(' + PositionerName + ',' +
                   str(ClosedLoopStatus) + ',' + str(KP) + ',' + str(KI) + ',' + str(KD) +
                   ',' + str(KS) + ',' + str(IntegrationTime) + ',' +
                   str(DerivativeFilterCutOffFrequency) + ',' + str(GKP) + ',' + str(GKI) +
                   ',' + str(GKD) + ',' + str(KForm) + ',' + str(KFeedForwardAcceleration) +
    ...
    # Update corrector parameters
    def PositionerCorrectorP2IDFFAccelerationSet(self, PositionerName, ClosedLoopStatus, KP, KI,
                                                 KI2, KD, KS, IntegrationTime,
                                                 DerivativeFilterCutOffFrequency, GKP, GKI, GKD,
                                                 KForm, KFeedForwardAcceleration, KFeedForwardJerk,
                                                 SetpointPositionDelay):
        command = ('PositionerCorrectorP2IDFFAccelerationSet(' + PositionerName + ',' +
                   str(ClosedLoopStatus) + ',' + str(KP) + ',' + str(KI) + ',' + str(KI2) +
                   ',' + str(KD) + ',' + str(KS) + ',' + str(IntegrationTime) + ',' +
                   str(DerivativeFilterCutOffFrequency) + ',' + str(GKP) + ',' + str(GKI) +
                   ',' + str(GKD) + ',' + str(KForm) + ',' + str(KFeedForwardAcceleration) +
                   ',' + str(KFeedForwardJerk) + ',' + str(SetpointPositionDelay) + ')')
        return self.__sendAndReceive(command)
    ...
    # Update corrector parameters
    def PositionerCorrectorPIDFFVelocitySet(self, PositionerName, ClosedLoopStatus, KP, KI, KD,
                                            KS, IntegrationTime, DerivativeFilterCutOffFrequency,
                                            GKP, GKI, GKD, KForm, KFeedForwardVelocity):
        command = ('PositionerCorrectorPIDFFVelocitySet(' + PositionerName + ',' +
                   str(ClosedLoopStatus) + ',' + str(KP) + ',' + str(KI) + ',' + str(KD) +
                   ',' + str(KS) + ',' + str(IntegrationTime) + ',' +
                   str(DerivativeFilterCutOffFrequency) + ',' + str(GKP) + ',' + str(GKI) +
                   ',' + str(GKD) + ',' + str(KForm) + ',' + str(KFeedForwardVelocity) + ')')
        return self.__sendAndReceive(command)
    ...
    # Update corrector parameters
    def PositionerCorrectorPIDDualFFVoltageSet(self, PositionerName, ClosedLoopStatus,
                                               KP, KI, KD, KS, IntegrationTime,
                                               DerivativeFilterCutOffFrequency, GKP, GKI,
                                               GKD, KForm, KFeedForwardVelocity,
                                               KFeedForwardAcceleration, Friction):
        command = ('PositionerCorrectorPIDDualFFVoltageSet(' + PositionerName + ',' +
                   str(ClosedLoopStatus) + ',' + str(KP) + ',' + str(KI) + ',' + str(KD) +
                   ',' + str(KS) + ',' + str(IntegrationTime) + ',' +
                   str(DerivativeFilterCutOffFrequency) + ',' + str(GKP) + ',' + str(GKI) +
                   ',' + str(GKD) + ',' + str(KForm) + ',' + str(KFeedForwardVelocity) + ',' +
                   str(KFeedForwardAcceleration) + ',' + str(Friction) + ')')
        return self.__sendAndReceive(command)
    ...
    # Update corrector parameters
    def PositionerCorrectorPIPositionSet(self, PositionerName, ClosedLoopStatus, KP, KI,
                                         IntegrationTime):
        command = ('PositionerCorrectorPIPositionSet(' + PositionerName + ',' +
    ...
    def PositionerCorrectorPIPositionGet(self, PositionerName):
        command = ('PositionerCorrectorPIPositionGet(' + PositionerName +
                   ',bool *,double *,double *,double *)')
        return self.__sendAndReceiveWithDecode(command)

    # Update corrector parameters
    def PositionerCorrectorSR1AccelerationSet(self, PositionerName, ClosedLoopStatus, KP, KI, KV,
                                              ObserverFrequency, CompensationGainVelocity,
                                              CompensationGainAcceleration, CompensationGainJerk):
        command = ('PositionerCorrectorSR1AccelerationSet(' + PositionerName + ',' +
                   str(ClosedLoopStatus) + ',' + str(KP) + ',' + str(KI) + ',' + str(KV) +
                   ',' + str(ObserverFrequency) + ',' + str(CompensationGainVelocity) + ',' +
                   str(CompensationGainAcceleration) + ',' + str(CompensationGainJerk) + ')')
        return self.__sendAndReceive(command)
    ...
    def PositionerCorrectorSR1ObserverAccelerationSet(self, PositionerName, ParameterA,
                                                      ParameterB, ParameterC):
        command = ('PositionerCorrectorSR1ObserverAccelerationSet(' + PositionerName + ',' +
                   str(ParameterA) + ',' + str(ParameterB) + ',' + str(ParameterC) + ')')
        return self.__sendAndReceive(command)

    # Read SR1 corrector observer parameters
    def PositionerCorrectorSR1ObserverAccelerationGet(self, PositionerName):
        command = ('PositionerCorrectorSR1ObserverAccelerationGet(' + PositionerName +
                   ',double *,double *,double *)')
        return self.__sendAndReceiveWithDecode(command)

    # Update SR1 corrector output acceleration offset
    def PositionerCorrectorSR1OffsetAccelerationSet(self, PositionerName, AccelerationOffset):
        command = ('PositionerCorrectorSR1OffsetAccelerationSet(' + PositionerName + ',' +
    ...
    def PositionerCurrentVelocityAccelerationFiltersSet(self, PositionerName,
                                                        CurrentVelocityCutOffFrequency,
                                                        CurrentAccelerationCutOffFrequency):
        command = ('PositionerCurrentVelocityAccelerationFiltersSet(' + PositionerName + ',' +
    ...
    def PositionerCurrentVelocityAccelerationFiltersGet(self, PositionerName):
        command = ('PositionerCurrentVelocityAccelerationFiltersGet(' + PositionerName +
                   ',double *,double *)')
        return self.__sendAndReceiveWithDecode(command)
    ...
    def PositionerDriverFiltersSet(self, PositionerName, KI, NotchFrequency,
                                   NotchBandwidth, NotchGain, LowpassFrequency):
        command = ('PositionerDriverFiltersSet(' + PositionerName + ',' + str(KI) + ',' +
                   str(NotchFrequency) + ',' + str(NotchBandwidth) + ',' + str(NotchGain) +
    ...
    def PositionerEncoderAmplitudeValuesGet(self, PositionerName):
        command = ('PositionerEncoderAmplitudeValuesGet(' + PositionerName +
                   ',double *,double *,double *,double *)')
        return self.__sendAndReceiveWithDecode(command)

    # Read analog interpolated encoder calibration parameters
    def PositionerEncoderCalibrationParametersGet(self, PositionerName):
        command = ('PositionerEncoderCalibrationParametersGet(' + PositionerName +
                   ',double *,double *,double *,double *)')
        return self.__sendAndReceiveWithDecode(command)
    ...
    def PositionerExcitationSignalGet(self, PositionerName):
        command = ('PositionerExcitationSignalGet(' + PositionerName +
                   ',int *,double *,double *,double *)')
        return self.__sendAndReceiveWithDecode(command)

    # Set excitation signal mode
    def PositionerExcitationSignalSet(self, PositionerName, Mode, Frequency, Amplitude, Time):
        command = ('PositionerExcitationSignalSet(' + PositionerName + ',' + str(Mode) + ',' +
                   str(Frequency) + ',' + str(Amplitude) + ',' + str(Time) + ')')
        return self.__sendAndReceive(command)
    ...
    def PositionerHardInterpolatorFactorSet(self, PositionerName, InterpolationFactor):
        command = ('PositionerHardInterpolatorFactorSet(' + PositionerName +
    ...
    def PositionerMaximumVelocityAndAccelerationGet(self, PositionerName):
        command = ('PositionerMaximumVelocityAndAccelerationGet(' + PositionerName +
                   ',double *,double *)')
        return self.__sendAndReceiveWithDecode(command)
    ...
    def PositionerMotionDoneSet(self, PositionerName, PositionWindow, VelocityWindow,
                                CheckingTime, MeanPeriod, TimeOut):
        command = ('PositionerMotionDoneSet(' + PositionerName + ',' + str(PositionWindow) +
                   ',' + str(VelocityWindow) + ',' + str(CheckingTime) + ',' +
                   str(MeanPeriod) + ',' + str(TimeOut) + ')')
        return self.__sendAndReceive(command)
    ...
    def PositionerPositionCompareAquadBWindowedGet(self, PositionerName):
        command = ('PositionerPositionCompareAquadBWindowedGet(' + PositionerName +
                   ',double *,double *,bool *)')
        return self.__sendAndReceiveWithDecode(command)

    # Set position compare AquadB windowed parameters
    def PositionerPositionCompareAquadBWindowedSet(self, PositionerName, MinimumPosition,
                                                   MaximumPosition):
        command = ('PositionerPositionCompareAquadBWindowedSet(' + PositionerName + ',' +
    ...
    def PositionerPositionCompareSet(self, PositionerName, MinimumPosition,
                                     MaximumPosition, PositionStep):
        command = ('PositionerPositionCompareSet(' + PositionerName + ',' + str(MinimumPosition) +
                   ',' + str(MaximumPosition) + ',' + str(PositionStep) + ')')
        return self.__sendAndReceive(command)
    ...
    def PositionerPositionComparePulseParametersGet(self, PositionerName):
        command = ('PositionerPositionComparePulseParametersGet(' + PositionerName +
                   ',double *,double *)')
        return self.__sendAndReceiveWithDecode(command)

    # Set position compare PCO pulse parameters
    def PositionerPositionComparePulseParametersSet(self, PositionerName, PCOPulseWidth,
                                                    EncoderSettlingTime):
        command = ('PositionerPositionComparePulseParametersSet(' + PositionerName + ',' +
                   str(PCOPulseWidth) + ',' + str(EncoderSettlingTime) + ')')
        return self.__sendAndReceive(command)

    # Get position compare scan acceleration limit
    def PositionerPositionCompareScanAccelerationLimitGet(self, PositionerName):
        command = ('PositionerPositionCompareScanAccelerationLimitGet(' + PositionerName +
                   ',double *)')
        return self.__sendAndReceiveWithDecode(command)

    # Set position compare scan acceleration limit
    def PositionerPositionCompareScanAccelerationLimitSet(self, PositionerName,
                                                          ScanAccelerationLimit):
        command = ('PositionerPositionCompareScanAccelerationLimitSet(' + PositionerName +
    ...
    def PositionerPreCorrectorExcitationSignalGet(self, PositionerName):
        command = ('PositionerPreCorrectorExcitationSignalGet(' + PositionerName +
                   ',double *,double *,double *)')
        return self.__sendAndReceiveWithDecode(command)

    # Set pre-corrector excitation signal mode
    def PositionerPreCorrectorExcitationSignalSet(self, PositionerName, Frequency, Amplitude, Time):
        command = ('PositionerPreCorrectorExcitationSignalSet(' + PositionerName + ',' +
                   str(Frequency) + ',' + str(Amplitude) + ',' + str(Time) + ')')
        return self.__sendAndReceive(command)

    # Get the raw encoder position
    def PositionerRawEncoderPositionGet(self, PositionerName, UserEncoderPosition):
        command = ('PositionerRawEncoderPositionGet(' + PositionerName + ',' +
    ...
    def PositionerSGammaExactVelocityAjustedDisplacementGet(self, PositionerName,
                                                            DesiredDisplacement):
        command = ('PositionerSGammaExactVelocityAjustedDisplacementGet(' + PositionerName +
                   ',' + str(DesiredDisplacement) + ',double *)')
        return self.__sendAndReceiveWithDecode(command)
    ...
    def PositionerSGammaParametersSet(self, PositionerName, Velocity, Acceleration,
                                      MinimumTjerkTime, MaximumTjerkTime):
        command = ('PositionerSGammaParametersSet(' + PositionerName + ',' +
    ...
    def PositionerStageParameterSet(self, PositionerName, ParameterName, ParameterValue):
        command = ('PositionerStageParameterSet(' + PositionerName + ',' + ParameterName +
                   ',' + ParameterValue + ')')
        return self.__sendAndReceive(command)
    ...
    def PositionerTimeFlasherSet(self, PositionerName, MinimumPosition,
                                 MaximumPosition, TimeInterval):
        command = ('PositionerTimeFlasherSet(' + PositionerName + ',' + str(MinimumPosition) +
    ...
    def PositionerUserTravelLimitsSet(self, PositionerName, UserMinimumTarget, UserMaximumTarget):
        command = ('PositionerUserTravelLimitsSet(' + PositionerName + ',' +
    ...
    def PositionerWarningFollowingErrorSet(self, PositionerName, WarningFollowingError):
        command = ('PositionerWarningFollowingErrorSet(' + PositionerName + ',' +
    ...
    def PositionerCorrectorAutoTuning(self, PositionerName, TuningMode):
        command = ('PositionerCorrectorAutoTuning(' + PositionerName + ',' + str(TuningMode) +
                   ',double *,double *,double *)')
        return self.__sendAndReceiveWithDecode(command)
    ...
    def MultipleAxesPVTExecution(self, GroupName, TrajectoryFileName, ExecutionNumber):
        command = ('MultipleAxesPVTExecution(' + GroupName + ',' + TrajectoryFileName +
                   ',' + str(ExecutionNumber) + ')')
        return self.__sendAndReceive(command)
    ...
    def MultipleAxesPVTPulseOutputSet(self, GroupName, StartElement, EndElement, TimeInterval):
        command = ('MultipleAxesPVTPulseOutputSet(' + GroupName + ',' + str(StartElement) +
    ...
    def SingleAxisSlaveParametersSet(self, GroupName, PositionerName, Ratio):
        command = ('SingleAxisSlaveParametersSet(' + GroupName + ',' + PositionerName +
                   ',' + str(Ratio) + ')')
        return self.__sendAndReceive(command)
    ...
    def SpindleSlaveParametersSet(self, GroupName, PositionerName, Ratio):
        command = ('SpindleSlaveParametersSet(' + GroupName + ',' + PositionerName +
                   ',' + str(Ratio) + ')')
        return self.__sendAndReceive(command)
    ...
    def GroupSpinParametersSet(self, GroupName, Velocity, Acceleration):
        command = ('GroupSpinParametersSet(' + GroupName + ',' + str(Velocity) + ',' +
                   str(Acceleration) + ')')
        return self.__sendAndReceive(command)
    ...
    def XYLineArcExecution(self, GroupName, TrajectoryFileName, Velocity,
                           Acceleration, ExecutionNumber):
        command = ('XYLineArcExecution(' + GroupName + ',' + TrajectoryFileName + ',' +
                   str(Velocity) + ',' + str(Acceleration) + ',' + str(ExecutionNumber) + ')')
        return self.__sendAndReceive(command)
    ...
    # Configure pulse output on trajectory
    def XYLineArcPulseOutputSet(self, GroupName, StartLength, EndLength, PathLengthInterval):
        command = ('XYLineArcPulseOutputSet(' + GroupName + ',' + str(StartLength) + ',' +
                   str(EndLength) + ',' + str(PathLengthInterval) + ')')
        return self.__sendAndReceive(command)
    ...
    def XYPVTExecution(self, GroupName, TrajectoryFileName, ExecutionNumber):
        command = ('XYPVTExecution(' + GroupName + ',' + TrajectoryFileName + ',' +
                   str(ExecutionNumber) + ')')
        return self.__sendAndReceive(command)
    ...
    def XYPVTPulseOutputSet(self, GroupName, StartElement, EndElement, TimeInterval):
        command = ('XYPVTPulseOutputSet(' + GroupName + ',' + str(StartElement) + ',' +
                   str(EndElement) + ',' + str(TimeInterval) + ')')
        return self.__sendAndReceive(command)
    ...
    # Return corrected profiler positions
    def XYZGroupPositionCorrectedProfilerGet(self, GroupName, PositionX, PositionY, PositionZ):
        command = ('XYZGroupPositionCorrectedProfilerGet(' + GroupName + ',' + str(PositionX) +
                   ',' + str(PositionY) + ',' + str(PositionZ) + ',double *,double *,double *)')
        return self.__sendAndReceiveWithDecode(command)

    # Return PCO raw encoder positions
    def XYZGroupPositionPCORawEncoderGet(self, GroupName, PositionX, PositionY, PositionZ):
        command = ('XYZGroupPositionPCORawEncoderGet(' + GroupName + ',' + str(PositionX) +
                   ',' + str(PositionY) + ',' + str(PositionZ) + ',double *,double *,double *)')
        return self.__sendAndReceiveWithDecode(command)
    ...
    def PositionerMotorOutputOffsetSet(self, PositionerName, PrimaryDAC1, PrimaryDAC2,
                                       SecondaryDAC1, SecondaryDAC2):
        command = ('PositionerMotorOutputOffsetSet(' + PositionerName + ',' +
                   str(PrimaryDAC1) + ',' + str(PrimaryDAC2) + ',' +
                   str(SecondaryDAC1) + ',' + str(SecondaryDAC2) + ')')
        return self.__sendAndReceive(command)
