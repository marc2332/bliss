...
robodiff_log_formatter = logging.Formatter("%(name)s %(levelname)s %(asctime)s %(message)s")
...
RobotMotors = collections.namedtuple("robodiff_motors", "fshut chi phi sampx sampy phiy phiz apz bstopz DtoX focus y z dw phienc phiyenc phizenc")
...
class Robodiff(object):
    ...
    def __init__(self, name, config, config_objects=None):
        ...
        robodiff_log_handler = TimedRotatingFileHandler(config.get('flex_log_file'), when='midnight', backupCount=7)
    ...
    def connect(self):
        if self._config_objects is None:
            ...
            self.diag_n = 0
            self.pilatus_deadtime = float(static_config.get_config().get_config(self.pilatus.name)['deadtime'])       
    ...
    def prestart_robodiff(self, cellNumber, SampleNumber):
        self._simultaneous_move(self.phiz, 0, self.phiy, 0, self.bstopz, self.bstopz.limits()[0],self.sampx, 0, self.sampy, 0, self.phi, 0, self.apz, self.apz.limits()[0], self.DtoX, self.DtoX.limits()[1])
    ...
    def move_to_park(self):
        self._simultaneous_move(self.phiz, 0,
                                self.phiy, 0,
                                self.bstopz, self.bstopz.limits()[0],
                                self.sampx, 0,
                                self.sampy, 0,
                                self.phi, 0,
                                self.apz, self.apz.limits()[0],
                                self.DtoX, self.DtoX.limits()[1])
        self.robodiff_reset()
        self.wcid30m.set("lightin", 0)
    ...
    def move_to_focus(self):
        move_task = self._simultaneous_move(self.sampx, 0.88, self.sampy, -0.47, wait=False)
    ...
    def prepare_DC(self):
        move_task = self._simultaneous_move(self.sampx, 0.88,
                                            self.sampy, -0.47,
                                            self.DtoX, 235,
                                            self.apz, -20,
                                            self.bstopz, -20, wait=False)
        self._move_roboDiff_XYZ(-1.2,-1.17,1.66)
        self.lightin(False)
        move_task.get(timeout=30)
    ...
    def set_detcover(self, set_in=True):
        with gevent.Timeout(10, RuntimeError("Could not put detector %s" % ('in' if set_in else 'out'))):
            if set_in:
                self.detcover.set_in()
            else:
    ...
    def unload_sample(self, CellNumber, PuckNumber, SampleNumber, retry=True):
        ...
        with cleanup(self.update_motors): 
            ...
            if retry:
                ...
                n = 1
            for i in range(n):
                if self.dm_reader.sample_is_present(PuckNumber, SampleNumber) == (True, False):
                    break
                if self.dm_reader.sample_is_present(PuckNumber, SampleNumber) == (True, True):
                    if i == n-1:
                        raise RuntimeError("Vial present")
                if self.dm_reader.sample_is_present(PuckNumber, SampleNumber) == (False,False):
                    if i == n-1:
    ...
    def _load(self):
        ...
        log_msg = "(" + str(int(cell)) + ", " + str(int(puck)) + ", " + str(int(sample)) + ") "
    ...
    def _unload(self):
        ...
        if task_output == "ABORTED":
            ...
            self.set_sampleNumber(0)
            self.logfile(log_msg + "%s : Error --- Unload Crash in movefrombeam (Beam)" %self.user_name)
            self.crash += 1
            self.robot_init()
            return False
        elif task_output == "Crash - Vial lost in unload":
            self.set_sampleNumber(0)
            self.logfile(log_msg + "%s : Error --- Unload Crash in unload" %self.user_name)
            self.crash += 1
            self.robot_init()
            return False
        elif task_output == "Vial lost in movefrombeam":
            self.set_sampleNumber(0)
            self.logfile(log_msg + "%s : ERROR --- Unload Vial lost in movefrombeam" %self.user_name)
            return False
        elif task_output == "Sample ON":
            self.set_sampleNumber(0)
            ...
            self.set_sampleNumber(0)
            self.logfile(log_msg + "%s : ERROR --- Unload vial lost in unload" %self.user_name)
            return False
        elif task_output == "Cannot remove sample":
            self.set_sampleNumber(0)
            self.logfile(log_msg + "%s : ERROR --- Unload cannot remove sample in trash" %self.user_name)
            return False
        elif task_output == "Cannot remove sample in movefrombeam":
            self.set_sampleNumber(0)
            self.logfile(log_msg + "%s : ERROR --- Unload cannot remove sample in trash (movefrombeam)" %self.user_name)
            return False
    ...
    def cell_pos_init(self, cellNumber=0, robot_init=True):
        ...
        if robot_init:
            ...
            self._simultaneous_move(self.sampx, 0,
                                    self.sampy, 0,
                                    self.phiy, 0,
                                    self.phiz, 0,
                                    self.phi, 0, wait=True)
        if cellNumber == 0:
    ...
    def dw_check_pos(self):
        ...
        for i in range (1,11):
            ...
            print "Reference image center found at ", rot_trans[0], " pixels and ", rot_trans[1], " pixels"
    ...
    def _musst_prepare_newMUSST(self, e1, e2, esh1, esh2, pixel_detector_trigger_steps, exp_time):
        self.musst.ABORT
        self.musst.CLEAR
    ...
    def _musst_prepare(self, e1, e2, esh1, esh2, pixel_detector_trigger_steps, exp_time):
        self.musst.ABORT
        self.musst.CLEAR
        self.musst.putget("#HSIZE 0")
        self.musst.putget("#ESIZE 524288")
    ...
    def _helical_calc(self, helical_pos, exp_time):
        ...
        helical = { "phiy": {"trajectory": end.get("phiy", 0)-start.get('phiy', 0), "motor": self.phiy, "start":start_phiy },
                    "phiz": {"trajectory": end.get("phiz", 0)-start.get('phiz', 0), "motor": self.phiz, "start": start_phiz },
                    "sampx":{"trajectory": end["sampx"]-start["sampx"], "motor": self.sampx, "start":start.get("sampx") },
                    "sampy":{"trajectory": end["sampy"]-start["sampy"], "motor": self.sampy, "start":start.get("sampy") } }
        for motor_name in helical.keys():
            ...
            if hm["distance"]<=5E-3:
                ...
                continue
            hm["backlash"]=helical[motor_name]["motor"].controller.backlash(hm["motor"].name, hm["start"], hm["start"]+hm["trajectory"])
    ...
    def helical_oscil(self, omega_start_ang, omega_stop_ang, helical_pos, exp_time, nb_pass=1, save_diagnostic=True, operate_shutter=True, nb_lines=1):
        def oscil_cleanup(phi_v=self.phi.velocity(from_config=True), phi_a=self.phi.acceleration(from_config=True),
                          phiz_v=self.phiz.velocity(from_config=True), phiz_a=self.phiz.acceleration(from_config=True),
        ...
        if True:
            ...
            if dist_phiy < 0.05 and dist_phiz > 0.05:
                self.vert_mesh_still2(initial_phiy, final_phiy, initial_phiz, final_phiz, nb_images_per_line, expo, nb_lines)
            else:
                self.hor_mesh_still2(initial_phiy, final_phiy, initial_phiz, final_phiz, nb_images_per_line, expo, nb_lines)
    ...
    def _oscil_calc(self, start_ang, stop_ang, exp_time, nb_pass):
        ...
        acc_time = (self.phi.acctime(from_config=True))
    ...
    def oscil(self, start_ang, stop_ang, exp_time, nb_pass=1, save_diagnostic=True, operate_shutter=True, helical=False):
        while self.diode(True,False)[0] < 1e-08: # and self.PSS_State():
            print "Waiting for beam to be back (i0)"
            time.sleep(10)
        while True:
            try:
                ...
                if temperature > 140.0:
                    ...
                    raise RuntimeError("Cryostreaming warming up")
                break
        d, oscil_start, oscil_final, velocity, calc_velocity, acc_time, acc_ang = self._oscil_calc(start_ang, stop_ang, exp_time, nb_pass)
        ...
        with cleanup(oscil_cleanup):
            ...
            shutter_predelay_steps = math.fabs(float(self.shutter_predelay * velocity * encoder_step_size))
            shutter_postdelay_steps = math.fabs(float(self.shutter_postdelay * velocity * encoder_step_size))
            ...
            if operate_shutter:
                #DN NEW MUSST
                self._musst_prepare_newMUSST(e1, e2, esh1, esh2, pixel_detector_trigger_steps, exp_time)
    ...
    def _get_diagnostic(self, phi_encoder_pos):
        ...
        diag_data[1:,1]=[xi-prev_xi for xi, prev_xi in zip(data[:,2][1:], data[:,2])]
        diag_data[1:,1]/=[float(ti-prev_ti) for ti, prev_ti in zip(data[:,0][1:], data[:,0])]
    ...
    def merge_diag(self, enc_pos=0):
        ...
        with open(self.diagfile, "a+") as diagfile:
            diagfile.write("\n#S %d\n#D %s\n" % (self.diag_n, datetime.datetime.now().strftime("%a %b %d %H:%M:%S %Y")))
            diagfile.write("#N %d\n" % cols)
            diagfile.write("#L Time(ms)  SampX  SampY  PhiY  PhiZ  Detector Acquiring\n")
    ...
    def save_diagnostic(self, phi_encoder_pos=0):
        ...
        with open(self.diagfile, "a+") as diagfile:
            diagfile.write("\n#S %d\n#D %s\n" % (self.diag_n, datetime.datetime.now().strftime("%a %b %d %H:%M:%S %Y")))
            diagfile.write("#N %d\n" % cols)
            diagfile.write("#L Time(ms)  Speed  Phi  I0  I1  Shut Cmd  Shut State  Detector Acquiring\n")
            numpy.savetxt(diagfile, diag_data)
            diagfile.write("\n\n")
    ...
    def musst_mesh(self, e1, e2, esh1, esh2, edet, exp_time, nb_images_per_line, nb_lines, direction):
        time.sleep(2)
        self.musst.ABORT
        self.musst.CLEAR
        self.musst.putget("#HSIZE 0")
    ...
    def musst1_mesh2(self, e1, e2, esh1, esh2, edet, delta, exp_time, nb_images_per_line, direction):
        self.musst.ABORT
        self.musst.CLEAR
        self.musst.putget("#HSIZE 0")
        self.musst.putget("#ESIZE 524288")
    ...
    def powder_mesh(self):
      ...
      for j in numpy.arange(-.075,.125,.050):
          ...
          for i in numpy.arange(0,183.6, 3.6):
              ...
              self.vert_mesh_still2(j, j, -0.3, 0.3, 60, 0.1, 1, "/lbsram/data/id30a1/inhouse/opid30a1/didier/69-3b", name)

    def vert_mesh_still2(self, phiy_initial=-0.5, phiy_final=0.5, phiz_initial=-0.05, phiz_final=0.05, nb_images_per_line=33, expo=0.0333, nb_lines=1, dir_name ="/lbsram/didier", prefix="test"):
        self.musst.ABORT
        self.musst2.ABORT
        phiz_step_size = self.phiz.steps_per_unit
        phiz_vel = abs((phiz_final - phiz_initial) / (nb_images_per_line * (expo + self.pilatus_deadtime)))
        #phiz_acc_dist = (self.phiz.acceleration(from_config =True) * phiz_vel) / 2
        phiz_acc_dist = (self.phiz.acctime(from_config=True) * phiz_vel) / 2
        delta = expo / 10.
        ...
        for phiy_pos in numpy.linspace(phiy_initial, phiy_final, nb_lines):
            ...
            e1, e2, esh1, esh2, edet = self.vert_mesh_calc(phiz_start, phiz_initial, phiz_final, phiz_end, phiz_vel)
            print e1, e2, esh1, esh2, edet, delta
            self.musst1_mesh2(e1, e2, esh1, esh2, edet, delta, expo, nb_images_per_line, "vert")
    ...
    def hor_mesh_still2(self, phiy_initial=-0.05, phiy_final=0.05, phiz_initial=-0.5, phiz_final=0.5, nb_images_per_line=33, expo=0.0333, nb_lines=3, dir_name ="/lbsram/didier", prefix="test"):
        self.musst.ABORT
        self.musst2.ABORT
        ...
        phiy_vel = abs((phiy_final - phiy_initial) / (nb_images_per_line * (expo + self.pilatus_deadtime)))
        #phiy_acc_dist = (self.phiy.acceleration(from_config =True) * phiy_vel) / 2
        phiy_acc_dist = (self.phiy.acctime(from_config=True) * phiy_vel) / 2
        phiy_start = phiy_initial + d * (phiy_acc_dist + self.acc_margin / phiy_step_size)
        ...
        for phiz_pos in numpy.linspace(phiz_initial, phiz_final, nb_lines):
            ...
            if self.acc_margin  < (self.shutter_predelay * phiy_vel * phiy_step_size):
                return "acc_margin too short"
            e1, e2, esh1, esh2, edet = self.hor_mesh_calc(phiy_start, phiy_initial, phiy_final, phiy_end, phiy_vel)
            print e1, e2, esh1, esh2, edet, delta
            self.musst1_mesh2(e1, e2, esh1, esh2, edet, delta, expo, nb_images_per_line, "hor")
            self.musst2_mesh2(delta, nb_images_per_line)
    ...
    def mesh_still(self, phiy_initial=-0.05, phiy_final=0.05, phiz_initial=-0.5, phiz_final=0.5, nb_images_per_line=33, expo=0.0333, nb_lines=3, dir_name ="/lbsram/didier", prefix="test"):
        self.ltrack_disable()
        ...
        self.phiy.move(phiy_start)
        self.phiy.velocity(phiy_vel) # * phiy_step_size)
        phiy_end = phiy_start + (2 * (phiy_acc_dist  + self.acc_margin / phiy_step_size) + (phiy_final - phiy_initial))
        if (d * phiy_start * phiy_step_size) < (d * phiy_end * phiy_step_size):
            ...
            self.phiy.controller._icepap_comm("#%%d:LISTDAT %d %d 2" %(d * phiy_start * phiy_step_size, d * phiy_end * phiy_step_size), self.phiy.name)
            ...
            self.phiy.controller._icepap_comm("#%%d:LISTDAT %d %d 2" %(d * phiy_end * phiy_step_size, d * phiy_start * phiy_step_size), self.phiy.name)
            self.phiy.controller._icepap_comm("#%d:LTRACK INPOS", self.phiy.name)
        else:
        ...
        self.phiz.controller._icepap_comm("#%%d:LISTDAT %d %d %d" %(d * phiz_initial * phiz_step_size, d * phiz_final * phiz_step_size, nb_lines + 1), self.phiz.name)
        ...
        if self.acc_margin  < (self.shutter_predelay * phiy_vel * phiy_step_size):
            return "acc_margin too short"
        e1, e2, esh1, esh2, edet = self.mesh_calc(phiy_start, phiy_initial, phiy_final, phiy_end, phiy_vel)
    ...
    def save_diagnostic_SRX(self,diagfile):
        ...
        with open(diagfile, "a+") as diagfile:
            diagfile.write("\n#S %d\n#D %s\n" % (self.diag_n, datetime.datetime.now().strftime("%a %b %d %H:%M:%S %Y")))
            diagfile.write("#N %d\n" % cols)
            diagfile.write("#L Time(ms)  Shutter State  Det Acq.  I0  I1  Shutter/Det. Cmd\n")
            numpy.savetxt(diagfile, diag_data)
            diagfile.write("\n\n")
    ...
    def SRX_prepare(self):
        self._simultaneous_move(self.phiy, self.phiy.limits()[0], self.phiz, self.phiz.limits()[0])
    ...
    def SRX_DC(self, aperture = "10 um", length_in_mm=2.4, height_in_mm=2.4, space_in_um=40.0, dir_name="/lbsram/data/visitor/mx1676/id30a1/20150728/cellB", img_name="SRX", nb_images=150, exp_time=0.02):
        diag_task = None
        n_height = 1
        ...
        array_height = numpy.arange(self.phiz.limits()[0], self.phiz.limits()[0] + height_in_mm, space_in_um/1000.0)
        array_length = numpy.arange(self.phiy.limits()[0], self.phiy.limits()[0] + length_in_mm, space_in_um/1000.0)
        ...
        with open(os.path.join(os.environ["HOME"],"phi_test.dat"), "a+") as f:
            ...
            while(True):
                with open(os.path.join(os.environ["HOME"],"phi_test.dat"), "a+") as f:
                    ...
                    f.write("phi position (icepap controller)= %s   encoder position (icepap controller)= %s  encoder position (musst) %s \n" %(phi_pos, enc_pos_icepap, enc_pos_musst))
                    self.phi.move(180)
                    phi_pos        = self.phi.position()
                    enc_pos_icepap = self.phienc.read()*self.phienc.steps_per_unit
                    enc_pos_musst  = self.musst.putget("#?CH CH2")[:-3]
    ...
    def beam_profile (self, length_in_mm=0.5, height_in_mm=0.5, space_in_um=5.0, filename='test.dat'):
        array_height = numpy.arange(self.phiz.limits()[1]-height_in_mm,self.phiz.limits()[1], space_in_um/1000.0)
        array_length = numpy.arange(self.phiy.limits()[1] - length_in_mm, self.phiy.limits()[1], space_in_um/1000.0)
        for i in array_height:
            n_line = 1
            self.phiz.move(i)
            for j in array_length:
    ...
    def estimateFocus(self, filename, imgview=False):
        ...
        img = scipy.misc.imread(filename, True)
        shape = img.shape
        img_crop = img[(shape[0]/2-30):(shape[0]/2+30), (shape[1]/2-150):(shape[1]/2+150)]
        shape = img_crop.shape
        if imgview:
            scipy.misc.imshow(img_crop)
        nr = numpy.sqrt(shape[0] * shape[0] + shape[1] * shape[1]) // 2
    ...
    def autofocus_test(self, size=0.5, step=20.0):
        ...
        pos = 0.3
        data_X=numpy.arange(pos-size/2, pos+(size/2.0)*(1.0+1.0/step),step_size)
        data_Y=numpy.array([])
        name = "/tmp/image"
        for i in data_X:
