...
static const uint8_t        wt_s_si = 0x02;     // 1MHz
static const uint8_t        cs_s_en = 0x00;     // by software
...
int main ( int argc, char ** argv )
{
  ...
  for ( ; ; ) {
    ...
    int poll_wait_rv;
    if ( (poll_wait_rv = ::epoll_wait(poll_fd, &pev, 1, timeout)) == -1 ) {
    ...
    if ( poll_wait_rv == 0 ) {
      if (timeout > 0) {
	reg = 1<<0;
	if ( !wr(device_fd, ct2::soft_start_stop, reg) )
	  return 35;
	timeout = -1;
      }
      else {
    ...
    switch ( fdh(pev.events) ) {
      ...
      case 2: rv = 36; goto clean_up;
      default: rv = 37; goto clean_up;
    }
