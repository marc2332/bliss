...
def wait_scan_finished(scans, writer, timeout=10):
    """
    :param list(bliss.scanning.scan.Scan) scans:
    :param PopenGreenlet writer: writer process
    :param num timeout:
    """
    done = {DevState.OFF, DevState.FAULT}
    with gevent.Timeout(timeout):
        for scan in scans:
            while writer.proxy.scan_state(scan.node.name) not in done:
                gevent.sleep(0.1)
...
def wait_scan_data_exists(scans, writer=None, timeout=10):
    """
    :param list(bliss.scanning.scan.Scan) scans:
    :param PopenGreenlet writer: writer process
    :param num timeout:
