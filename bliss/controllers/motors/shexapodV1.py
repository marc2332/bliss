...
class TurboPmacCommand(object):
    ...
    def __call__(self, command, convtype=None):
        return self.__send(self.VR_DOWNLOAD, self.VR_PMAC_GETRESPONSE, command, convtype)
...
class HexapodProtocolV1(BaseHexapodProtocol):
    ...
    SYSTEM_STATUS_FIELDS = ("error", "ready", "in_position", 
                            "control", "homing_done",
                            "brake_control", "emergency_stop_pressed",
                            "following_warning", "following_error",
                            "actuators_out", "amplifier_error",
    ...
    AXIS_STATUS_FIELDS = ("in_position", "control",
                          "homing_done", "homing_hardware_input",
                          "positive_hardware_lmit", "negative_hardware_limit",
                          "brake_control", "following_warning",
                          "following_error", "actuators_out",
                          "amplifier_error", "encoder_error",
                          "phasing_error")
    ...
    def speeds(self):
        ...
        return dict(tspeed= speeds[0], rspeed= speeds[1],
                    min_tspeed= speeds[2], max_tspeed= speeds[4],
                    min_rspeed= speeds[3], max_rspeed= speeds[5])
...
"""
    ...
    def dump_status(self):
        ...
        rows= [[ field ] + [ str(getattr(one_axis, field)) for one_axis in axis_status[0:6] ]
